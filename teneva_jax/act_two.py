...
def accuracy(Y1, Y2):
    ...
    Returns:
        jnp.ndarray of size 1: the relative difference between two tensors.
    ...
    if jnp.isinf(c) or jnp.isinf(z1) or jnp.isinf(z2) or abs(z2) < 1.E-100:
        return -1 # TODO: check
...
def add(Y1, Y2):
    ...
    def body(q, data):
        ...
        Z1 = jnp.zeros([r1_l, n, r2_r])
        Z2 = jnp.zeros([r2_l, n, r1_r])

        L1 = jnp.concatenate([G1, Z1], axis=2)
        L2 = jnp.concatenate([Z2, G2], axis=2)

        G = jnp.concatenate([L1, L2], axis=0)
    ...
    Yl = jnp.concatenate([Yl1, Yl2], axis=2)
    _, Ym = jax.lax.scan(body, None, (Ym1, Ym2))
    Yr = jnp.concatenate([Yr1, Yr2], axis=0)
...
def mul_scalar(Y1, Y2):
    ...
    Returns:
        jnp.ndarray of size 1: the scalar product.
...
def mul_scalar_stab(Y1, Y2):
    ...
    Returns:
        (jnp.ndarray of size 1, jnp.ndarray): the scaled value of the scalar
    ...
    q, pl = body(jnp.ones(1), (Yl1, Yl2))
    q, pm = jax.lax.scan(body, q, (Ym1, Ym2))
    q, pr = body(q, (Yr1, Yr2))

    return q, jnp.hstack((pl, pm, pr))
