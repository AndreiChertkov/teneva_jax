...
def maxvol(A, e=1.05, k=100):
    ...
    Returns:
        (jnp.ndarray, jnp.ndarray): the row numbers I containing the maximal
    ...
    Note:
        ...
        Matrix Methods: Theory, Algorithms And Applications: Dedicated to the
        Memory of Gene Golub (2010): 247-256.
    ...
    def cond(data):
        I, B, b, k_cur = data
        return jnp.logical_and(k_cur < k, jnp.abs(b) > e)
...
def maxvol_rect(A, e=1.1, dr_min=0, dr_max=None, e0=1.05, k0=10):
    ...
    Returns:
        (jnp.ndarray, jnp.ndarray): the row numbers I containing the rectangular
    ...
    I = jnp.hstack([I0, jnp.zeros(r_max-r, dtype=I0.dtype)])
    S = jnp.ones(n, dtype=I0.dtype)
    S.at[I0].set(0.)
    ...
    def cond(data):
        ...
        return jnp.logical_and(k_cur < r_max, f > e*e)
