...
def als(I_trn, y_trn, Y0, nswp=50):
    ...
    Args:
        I_trn (jnp.ndarray): multi-indices for the tensor in the form of array
            of the shape [samples, d], where d is a number of tensor's
            dimensions and samples is a size of the train dataset.
        y_trn (jnp.ndarray): values of the tensor for multi-indices I_trn in
...
def _build_indices(I_trn, n):
    ...
    for k in range(d):
        inds.append([])
        for j in range(n):
            inds[-1].append(jnp.where(I_trn[k, :] == j)[0])
...
def _iter_ltr(Yl, Ym, Z_rtl, I_trn, y_trn, inds):
    ...
    yl_trn, ym_trn = y_trn, jnp.repeat(y_trn[:, None], d-2, axis=1).T
    indsl, indsm = inds[0], inds[1:-1]

    _, (Yl, Zl_ltr) = _body_ltr(
        jnp.ones((m, 1)), (Yl, Z_rtl[0], Il_trn, yl_trn, indsl))
...
def _iter_rtl(Ym, Yr, Z_ltr, I_trn, y_trn, inds):
    ...
    ym_trn, yr_trn = jnp.repeat(y_trn[:, None], d-2, axis=1).T, y_trn
    indsm, indsr = inds[1:-1], inds[-1]

    _, (Yr, Zr_rtl) = _body_rtl(
        jnp.ones((1, m)), (Yr, Z_ltr[1], Ir_trn, yr_trn, indsr))
...
def _body_ltr(Z_ltr, data):
    ...
    G = G.swapaxes(0, 1)
    Z_ltr = jnp.einsum('mq,qmr->mr', Z_ltr, G[:, i, :])
...
def _body_rtl(Z_rtl, data):
    ...
    _, G = jax.lax.scan(_optimize, (Z_ltr, Z_rtl, y), (G, inds))
    G = G.swapaxes(0, 1)
    Z_rtl = jnp.einsum('rmq,qm->rm', G[:, i, :], Z_rtl)
...
def _optimize(args, data):
    ...
    lhs = Z_rtl[:, idx].T[:, jnp.newaxis, :]
    rhs = Z_ltr[idx, :][:, :, jnp.newaxis]
    ...
    sol = jnp.linalg.lstsq(AtA + lamb * jnp.identity(A.shape[1]), Aty)[0]
